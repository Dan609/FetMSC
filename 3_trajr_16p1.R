...
#alltracks_p16C <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), mynames)
#alltracks_p16L <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), mynames)
#alltracks_p16Y <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), mynames)


#alltracks_p16C$probe <- as.character(alltracks_p16C$probe)
#alltracks_p16L$probe <- as.character(alltracks_p16L$probe)
#alltracks_p16Y$probe <- as.character(alltracks_p16Y$probe)

alltracks$probe <- as.character(alltracks$probe)

#tracks <- alltracks
tracks_p16C <- alltracks # 
tracks_p16L <- alltracks # 
tracks_p16Y <- alltracks # 
...
#################################################
# Choose dir ---------
file_list_p16C <- list.files(path = , choose.dir(),
                           pattern = "csv",
                           all.files = FALSE,
...
file_list_p16L <- list.files(path = , choose.dir(),
                             pattern = "csv",
                             all.files = FALSE,
                             full.names = TRUE, recursive = TRUE,
                             ignore.case = FALSE, include.dirs = FALSE,
...
file_list_p16Y <- list.files(path = , choose.dir(),
                             pattern = "csv",
                             all.files = FALSE,
                             full.names = TRUE, recursive = TRUE,
                             ignore.case = FALSE, include.dirs = FALSE,
...
for (file_name in file_list_p16C) {
  tracks_p16C <- rbind(tracks_p16C, 
                       traj_analysis_p16C(file_name))}


# Start scan for LPA
for (file_name in file_list_p16L) {
  tracks_p16L <- rbind(tracks_p16L, 
                          traj_analysis_p16L(file_name))}


# Start scan for Y27632
for (file_name in file_list_p16Y) {
  tracks_p16Y <- rbind(tracks_p16Y, 
                          traj_analysis_p16Y(file_name))}
...
#################################################
# Merge all tracks-----------------------
alltracks <- rbind(tracks_p16C, tracks_p16L, tracks_p16Y) # collect all tracks
...
head(data)
tail(data)
write.csv(data, file = 'data16_1.csv')
write.xlsx(data, file = 'data16_serum1.xlsx')
