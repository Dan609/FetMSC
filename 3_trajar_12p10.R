...
#################################################
# Choose dir ---------
file_list_p12C <- list.files(path = , choose.dir(),
                             pattern = "csv",
                             all.files = FALSE,
...
for (file_name in file_list_p12C) {
  tracks_p12C <- rbind(tracks_p12C, 
                       traj_analysis_p12C(file_name))}
...
#################################################
# Merge all tracks-----------------------
alltracks <- rbind(tracks_p12C, tracks_p12L, tracks_p12Y) # collect all tracks
...
head(data)
tail(data)
write.csv(data, file = 'data12_10.csv')
write.xlsx(data, file = 'data12_serum10.xlsx')
