# Kirill's cells movement trajectory analysis created by Dan Bobkov, 2022 # dan.bobkov@gmail.com
...
#alltracks_p20C <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), mynames)
#alltracks_p20L <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), mynames)
#alltracks_p20Y <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), mynames)


#alltracks_p20C$probe <- as.character(alltracks_p20C$probe)
#alltracks_p20L$probe <- as.character(alltracks_p20L$probe)
#alltracks_p20Y$probe <- as.character(alltracks_p20Y$probe)

alltracks$probe <- as.character(alltracks$probe)

#tracks <- alltracks
tracks_p20C <- alltracks # 
tracks_p20L <- alltracks # 
tracks_p20Y <- alltracks # 
...
#################################################
# Choose dir ---------
file_list_p20C <- list.files(path = , choose.dir(),
                             pattern = "csv",
                             all.files = FALSE,
...
file_list_p20L <- list.files(path = , choose.dir(),
                             pattern = "csv",
                             all.files = FALSE,
                             full.names = TRUE, recursive = TRUE,
                             ignore.case = FALSE, include.dirs = FALSE,
...
file_list_p20Y <- list.files(path = , choose.dir(),
                             pattern = "csv",
                             all.files = FALSE,
                             full.names = TRUE, recursive = TRUE,
                             ignore.case = FALSE, include.dirs = FALSE,
...
for (file_name in file_list_p20C) {
  tracks_p20C <- rbind(tracks_p20C, 
                       traj_analysis_p20C(file_name))}


# Start scan for LPA
for (file_name in file_list_p20L) {
  tracks_p20L <- rbind(tracks_p20L, 
                       traj_analysis_p20L(file_name))}


# Start scan for Y27632
for (file_name in file_list_p20Y) {
  tracks_p20Y <- rbind(tracks_p20Y, 
                       traj_analysis_p20Y(file_name))}
...
#################################################
# Merge all tracks-----------------------
alltracks <- rbind(tracks_p20C, tracks_p20L, tracks_p20Y) # collect all tracks
...
head(data)
tail(data)
write.csv(data, file = 'data20_1.csv')
write.xlsx(data, file = 'data20_serum1.xlsx')
